...
<Grid Grid.Row="1" x:Name="SpectrumSlidePanel" Height="140" ClipToBounds="True">
    ...
    <Grid Width="768" HorizontalAlignment="Center">
        ...
        <!-- 1. 測定グリッド罫線 (水平4段階 #38FFFFFF〜#20FFFFFF + 垂直6分割 #30FFFFFF) -->
...
### 5.2 ビジュアルエフェクト仕様
1. **測定グリッド罫線**:
   - 水平罫線: 4分割の基準ライン（`#38FFFFFF`, `#30FFFFFF`, `#28FFFFFF`, `#20FFFFFF`）。
   - 垂直罫線: 6分割の帯域ディバイダー（`#30FFFFFF`）。
2. **縦方向ネオングラデーション**:
...
3. **ネオングロー**: `DropShadowEffect` (Color: SpectrumShadowColor, BlurRadius: 10, ShadowDepth: 0, Opacity: 0.95)
4. **水面ミラーリフレクション**:
