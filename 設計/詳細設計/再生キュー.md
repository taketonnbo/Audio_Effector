...
キュー内の複数トラックで `IsPlaying == true` が残存する現象を防止し、次アルバム遷移時も正確に再生アイコンを表示するため、`PlayerControlViewModel.SyncTrackPlayingStates` において `PlaybackListTracks` と `PlayQueue` の両方を確実に同期更新：
