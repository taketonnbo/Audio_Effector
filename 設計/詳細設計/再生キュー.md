...
- **アルゴリズムの特徴と整合性**:
  1. $i < k$ のトラック（現在曲より前）: 未再生であれば「履歴」に入れず、キューから除外。
  2. $i > k$ のトラック（現在曲より後）: シャッフル再生によって既に再生済みのもの（`_playedTrackPaths` に登録）は「履歴」に残したままキュー側は穴あき（除外）。
  3. 未再生トラックのみが元のアルバム順（インデックス昇順）でキューに並び、違和感のないアルバム再生へシームレスに復元。
