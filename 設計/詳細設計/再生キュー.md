...
- **履歴追加条件（`AudioService` 側判定）**:
  - トラック自然完奏時（`OnTrackEnded`）: 即座に `TrackPlaybackEnded` を発火。
  - スキップ・停止・別曲再生遷移時（`PlayCurrent`, `Stop`, `SetPlaylist`）: 再生時間が 5.0 秒以上（`MinPlaybackSecondsForHistory = 5.0`）経過していた場合のみ発火（誤操作スキップの混入を防止）。
