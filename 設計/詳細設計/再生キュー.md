...
- 現在再生中曲の元インデックスを $k$ とする。
- 復元後キューの構成:
  $$Q_{restored} = [ T_k ] \cup \{ T_i \mid i > k \land T_i \notin \text{\_playedTrackPaths} \}$$
...
private void RestorePlaylist()
{
    ...
    {
        // 1. 元のアルバム順（_originalPlaylist）において現在再生中の曲のインデックスを特定
        int originalIndex = _originalPlaylist.FindIndex(t => t.FilePath == currentTrack.FilePath);
        if (originalIndex >= 0)
        {
            // 2. 現在再生中曲を先頭に配置
            var restored = new List<Track> { currentTrack };

            // 3. 現在再生中曲より前の曲（先行アルバム含む）は除外
            // 4. 現在再生中曲より後の曲のうち、既に再生済み（_playedTrackPathsに含まれる）曲は穴あき除外
...
- **アルゴリズムの特徴と複数アルバム整合性**:
  1. **アルバム単位のグループ化維持**: `_originalPlaylist` に追加順（A → B → C）で各アルバムのトラック順が保持されているため、復元後は自然にアルバムごとにまとまった並び順になります。
  2. **現在曲より前の未再生曲の除外**: $i < k$ のトラック（先行するアルバムの未再生曲含む）は未再生であれば「履歴」に入れず、キューから除外。
  3. **後続の再生済み曲の穴あき除外**: $i > k$ のトラックのうち、シャッフル再生によって既に再生済みのもの（`_playedTrackPaths` に登録）は「履歴」に残したままキュー側は穴あき（除外）。
  4. 未再生トラックのみがアルバム順・トラック順で整然と並び、複数アルバムが混ざり合ったシャッフル状態から直感的なアルバムリスニング順へ完全復元。
