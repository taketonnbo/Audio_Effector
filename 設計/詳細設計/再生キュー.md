...
### 4.6 再生キュー空（Empty）時の再生停止および再生中情報初期化機構
...
3. **`PlayerControlViewModel.ClearQueue()` / `RemoveFromQueue()` の連動制御**:
   - `ClearQueue()` 呼び出し時、`PlayQueue.Clear()`、`_audioService.Stop(false)`、`CurrentTrack = null`、`TotalTimeDisplay = "00:00"`、`CurrentTimeDisplay = "00:00"`、`Progress = 0.0`、`NowPlayingImage = null` を即座に同期設定した上で `_audioService.SetPlaylist(new List<Track>())` を発行。
...
5. **`MainViewModel.OnTrackChanged(null)` の完全初期化**:
   - `track == null` を受信した場合、`CurrentTrack = null`, `CurrentAlbum = null`, `Progress = 0` とし、`NowPlayingImage` および `SpectrumBackgroundImage` をデフォルト画像にリセット。
