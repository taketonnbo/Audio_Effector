...
#### C. 再生終了曲の履歴追加およびキュー削除シーケンス（Issue #218連携）
```mermaid
sequenceDiagram
    autonumber
    ...
    Audio->>Audio: OnTrackEnded() または Next()
    Note over Audio: 終了トラック(endedTrack)を取得<br/>再生キュー(_playlist, _originalPlaylist)から削除
    Audio->>CtrlVM: TrackPlaybackEnded(endedTrack)
    Audio->>CtrlVM: PlaylistChanged(_playlist)
    Note over CtrlVM: RunOnUiThread<br/>1. PlayHistory.Insert(0, endedTrack) (重複時先頭移動、最大100件)<br/>2. PlayQueueを更新
    CtrlVM->>View: 再生リスト・履歴タブのUIを更新
```
...
- **キュー最終曲の終了判定（キュー残存時も次アルバムへ遷移）**:
  - `AudioService` は `SetPlaylist` 時に設定されたプレイリストの末尾曲（`_playlist[^1]`）を `_finalTrackOfQueue`（キュー最終曲）として保持。
  - 自然完奏時（`OnTrackEnded`）または手動スキップ時（`Next`）において、再生終了した曲が `_finalTrackOfQueue` と一致した場合、**リピート（ループ）時を除外し、キューの状態（他に曲が残っているか等）にかかわらず、キューの再生を終了として `PlaylistEnded` を発火**し、次アルバムへ確実に遷移。
  - `MainViewModel` は直前再生トラック（`_lastPlayedTrack`）およびアルバム（`_lastPlayedAlbum`）を記憶し、`CurrentTrack` が null の状態であっても直前のアルバムから次のアルバムを正しく特定して遷移を実行。
- **次アルバム自動移行時のイベント順序制御と停止通知抑止ガード**:
  - `AudioService.OnTrackEnded` および `Next` において、`PlaylistEnded` を発火した際、同期的に呼び出されるイベントハンドラ（`MainViewModel.OnPlaylistEnded` → `LibraryViewModel.PlayAlbum`）によって次のアルバムの再生（`_audioService.SetPlaylist`, `PlayTrack`）が開始されます。
  - ハンドラによって新しい再生が開始された場合（`_playlist.Count > 0 || _currentIndex >= 0 || _outputDevice != null`）、**後続の停止通知（`PlaybackStopped`, `PlaybackStateChanged(false)`, `TrackChanged(null)`）を抑止**することで、新しく開始された次アルバムの再生中状態（`CurrentTrack`, `track.IsPlaying`）が null や false に上書き破壊される不具合を根本から防止。
- **通常アルバム再生（`LibraryViewModel.PlayAlbum`）と同一パイプラインを通過**:
  - `_audioService.IsShuffleEnabled` を判定。
  - **シャッフルON時**: 次アルバムの収録曲の中からランダムに1曲を選定して `startTrack` とし、`startTrack.IsPlaying = true` を設定した上で `SetPlaylist(tracks, startTrack)` および `PlayTrack(startTrack)` を実行。これにより、**ランダムな1曲がキューの先頭（1行目）に固定されて再生され、残りの曲は2行目以降にシャッフル配置**されます（キューの途中から再生される不具合を防止）。
  - **シャッフルOFF時**: 先頭トラック（トラック1）から通常順序でキューに展開して再生開始。
  - `PlaybackRequested` を通じてメイン画面右ペイン（トラックリスト、アルバム名、アーティスト名）も次アルバムの情報に正しく同期更新。

### 4.4 再生中アイコン排他制御およびPlayQueue同期ロジック (Issue #214対策)
キュー内の複数トラックで `IsPlaying == true` が残存する現象を防止し、次アルバム遷移時も正確に再生アイコンを表示するため、以下の多重同期機構を実装：

1. **`PlayerControlViewModel.PlayQueue` setter での自動同期**:
   - `PlayQueue` プロパティに新しいコレクションが代入された際、セッター内で即座に `SyncTrackPlayingStates(CurrentTrack)` を実行し、代入直後のトラック群でも `CurrentTrack` と一致する曲の `IsPlaying` を直ちに `true` に同期。
2. **`MainViewModel.OnPlaylistChanged` での同期呼出**:
   - `PlaylistChanged` イベントによる `PlayQueue` の非同期更新時にも、`PlayerControl?.SyncTrackPlayingStates(CurrentTrack)` を呼んで同期を保証。
3. **`LibraryViewModel.PlayAlbum` での事前フラグ設定**:
   - アルバム再生開始時に `startTrack.IsPlaying = true` を事前設定することで、`PlayQueue` 生成時点での即時表示を徹底。
4. **`SyncTrackPlayingStates` による完全排他制御**:
   - `PlaybackListTracks` と `PlayQueue` の両方を走査し、現在の再生中トラックのみを `true`、それ以外を `false` に同期。
...
public void SyncTrackPlayingStates(Track? currentTrack)
{
    if (PlaybackListTracks != null)
    {
        foreach (var track in PlaybackListTracks)
        {
            ...
            if (track.IsPlaying != shouldBePlaying) track.IsPlaying = shouldBePlaying;
        }
    }
...
### 4.5 履歴管理および再生後キュー削除ロジック (Issue #218)
- `PlayHistory`: `ObservableCollection<Track>` として `PlayerControlViewModel` で保持。
- **再生終了イベント連携**: `IAudioService.TrackPlaybackEnded` イベントを購読し、通知されたトラックをUIスレッド（`RunOnUiThread`）で履歴に追加。
- **履歴追加および再生キュー削除条件（`AudioService` 側判定）**:
  - トラック自然完奏時（`OnTrackEnded`）または手動次へスキップ時（`Next`）:
    - 直前再生曲を `CheckAndPreparePlaybackEnded` で取得（再生時間制限なし、5秒未満のスキップも履歴追加対象）。
    - 取得した終了曲を再生キュー（`_playlist` および `_originalPlaylist`）から自動削除。
    - `PlaylistChanged` を通知してUIの再生キュー（再生リスト）から該当曲を除去。
    - `TrackPlaybackEnded` イベントを発火して履歴に追加。
  - 同一楽曲の二重通知防止（`_currentTrackReportedAsEnded` フラグによる排他制御）。
- **重複曲のハンドリング**: 同一ファイルパス（`FilePath`）の楽曲が既に履歴内に存在する場合、既存のエントリを削除した上で先頭に `Insert(0, track)` して最新化（重複排除・最新順管理）。
- **上限設定**: 最大保持件数は 100 件（`MaxHistoryCount = 100`）。100件を超過した場合は末尾（最も古い曲）を `RemoveAt(100)` で自動破棄。
- **履歴からの再生挙動**: 履歴の曲を選択・再生（`PlayFromHistoryCommand`）した際、該当曲を再生キューの最初（先頭）に追加（既存キューにあれば移動）し、自動的に「再生リスト」タブ（`SelectedQueueTabIndex = 0`）に切り替えて即座に再生を開始。
- **履歴アイテムのアイコン非表示**: 履歴タブ内のサムネイルでは、再生/一時停止アイコン（オーバーレイ）は表示せず、アルバムアートのみをシンプルに表示。
- **タブ連動クリア**: 再生キュースライドパネル上部の全クリアボタンは、現在選択中のタブ（`SelectedQueueTabIndex`）に応じて `ClearQueueCommand` または `ClearHistoryCommand` を実行する `ClearCurrentTabCommand` にバインド。
