...
3. **STAスレッドテストにおけるDispatcherシャットダウン必須化**:
   - テスト都合で本体コードを安易に変更するのではなく、テストコード側で `System.Windows.Application` を生成した際は、スレッド終了時に必ず `finally` 句で `Dispatcher.CurrentDispatcher.InvokeShutdown()` を呼び出し、Dispatcherリソースを明示的に解放・シャットダウンすること。
