...
│       ├── DeviceApplicationService.cs          # デバイス検出・ファイル転送等の操作
...
2. **イベントの発行 (Publish)**: **Application層 (ApplicationService)** のみが `IEventBus.Publish(e)` を呼び出して発行します（Domain層はEventBusに依存せず、100%純粋な状態を保ちます）。
