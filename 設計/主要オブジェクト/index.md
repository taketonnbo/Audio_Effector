...
本ディレクトリでは、コードベースの精査に基づき抽出された、Audio Effectorのコアオブジェクト（名詞）の定義、現状の実装実態（As-Is）、および相互関係をまとめています。

---

## 1. 現状の実装におけるコレクション / シングル対応表 (As-Is)

| オブジェクト名 | 英名 | 分類 | 現状のコレクション表示 (一覧) | 現状のシングル表示 (詳細) | 課題 / 今後の指針 (To-Be) | 詳細定義 |
| :--- | :--- | :--- | :--- | :--- | :--- | :---: |
| **楽曲** | Track | プライマリ | **未実装**（`AllSongsView` はComing Soon）。<br>実質的にはアルバム展開、プレイリスト内、キュー内でのみ一覧表示。 | **なし**（右カラムの再生情報やプロパティダイアログのみ）。 | 全曲データグリッド（コレクション）の新設、およびプロパティ/タグ編集ビューの提供。 | [01_楽曲_Track.md](01_楽曲_Track.md) |
