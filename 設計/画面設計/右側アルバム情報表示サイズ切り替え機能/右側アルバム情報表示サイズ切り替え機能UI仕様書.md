...
*   **① 左カラム（アートワーク・基本情報・楽曲操作コントロール）**:
    *   **アルバムアート**: 正方形アスペクト比を維持し、左カラムの幅いっぱいに大きく配置（角丸 `CornerRadius="8"`, ドロップシャドウ付与）。
    *   **アルバム情報テキスト**:
    ...
    *   **楽曲操作コントロールバー（最下部）**:
        *   **シャッフルボタン (`🔀`)**: 有効時シアン点灯
        *   **前へボタン (`⏮`)**
        *   **再生/一時停止/停止ボタン (`▶` / `⏸` / `⏹`)**: アクセントカラー丸形ボタン
        *   **次へボタン (`⏭`)**
        *   **リピートボタン (`🔁`)**: 全曲/1曲/OFFの切り替え（有効時シアン点灯）
        *   **再生リストキューボタン (`≡`)**: クリックでキューパネル/ダイアログを表示
        *   **曲名表示 & シークバー**: 現在再生中トラック名、現在時間/総時間（例: `00:59 / 04:34`）、シアンのプログレスバー
        *   **プレイリスト追加ボタン (`＋`)**: クリックでプレイリスト追加ダイアログを表示
        *   **お気に入りボタン (`♥`)**: お気に入り登録時ピンク/赤に点灯
        *   **最小化トグルボタン (`🗗` / `▼`)**: クリックで最小化モードへ切り替え
...
#### 📦 最小化表示時（コンパクトプレイヤー構成）
*   **上部コンパクトプレイヤー領域**:
    *   アルバムアートサムネイル（角丸 40x40 〜 60x60px）
    *   曲名・アーティスト名
    *   Hi-Resバッジ等の音質情報
    *   シークバー＆再生時間表示
    *   操作ボタン群（シャッフル、前へ、再生/一時停止、次へ、リピート、お気に入り）
    *   **最大化トグルボタン (`⛶` / `▲`)**: クリックで最大化モードへ切り替え
*   **下部拡張タブ領域**:
    *   右側パネルの残り領域全体（縦幅いっぱい）に拡張機能（リスニング統計・タグクラウド等）を展開。

---

### 3.3. モード切替ボタンスタイル
- **配置位置**: プレイヤーコントロールバーの右端、またはヘッダー右上
- **アイコン構成**:
  - 最大化時 → 縮小アイコン（`🗗` または `▼`）
  - 最小化時 → 拡大アイコン（`⛶` または `▲`）
- **カラーパレット**:
