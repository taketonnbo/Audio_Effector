...
- **トグルボタンクリック時**:
  ...
  - **閉じる（True -> False）**: 
    - 右側パネルの幅が滑らかに縮小（`1*` → `0*`、0.6秒）して右端へスライドアウトし、境界線（スプリッター領域）およびトグルボタンもパネルと完全に連動して右端へ移動します。
    - トップバー（コンパクトプレイヤー）が上部から滑らかに展開（高さ `0` → `60`、0.6秒）します。
  - **開く（False -> True）**: 
    - トップバーが上部へ折りたたまれて非表示（高さ `60` → `0`、0.6秒）になります。
    - 右側パネルの幅が画面右端から滑らかに拡大（`0*` → `1*`、0.6秒）してスライドインし、境界線およびトグルボタンもパネルの中身と完全に一体となって連動移動します。
