...
3. **リスト表示領域（UI仮想化）**:
   ...
   #### A. 再生リスト（キュー）表示時
   - **空状態（Empty State）**: キューが0件の場合は、音符アイコンと「再生キューは空です」「楽曲やアルバムを右クリックして『キューに追加』できます」というガイドメッセージを中央に淡く表示。また、キュー全クリアまたは全削除により空となった際は、音声を即座に完全停止し、プレイヤー側の現在再生中情報（トラック名、アーティスト名、カバーアート、再生時間・プログレス）およびメイン画面右側のプレイリスト（トラックリスト一覧、アルバムタイトル、曲数表示）を未選択・初期状態（空）へクリア表示。
   - **各トラックアイテムカード**:
   ...
   - **各履歴アイテムカード**:
     ...
     - **クリック時挙動**: 該当曲を再生キューの最初（先頭）に追加し、自動的に「再生リスト」タブへ切り替えて該当曲の再生を開始。
     - **右クリックコンテキストメニュー**:
