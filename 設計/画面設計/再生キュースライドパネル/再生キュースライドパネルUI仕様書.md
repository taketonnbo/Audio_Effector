...
3. **リスト表示領域（UI仮想化）**:
   ...
   #### A. 再生リスト（キュー）表示時
   - **空状態（Empty State）**: キューが0件の場合は、音符アイコンと「再生キューは空です」「楽曲やアルバムを右クリックして『キューに追加』できます」というガイドメッセージを中央に淡く表示。
   - **各トラックアイテムカード**:
...
- **シャッフルOFF（穴あき復元）**: 
  ...
  - 現在再生中楽曲を先頭に配置し、元収録順で現在曲より前の未再生曲は履歴に入れずキューから除外します。
  - シャッフル再生によって既に再生されて「履歴」に入っている曲は、履歴タブに残したままキュー側は穴あき（除外）とし、未再生の曲のみがアルバム順で並びます。
- **シャッフル中の追加**: 「次に再生」は現在曲の直後へ挿入。「キューに追加」は現在曲以外の全キューリストを再シャッフルしてランダム配置。
