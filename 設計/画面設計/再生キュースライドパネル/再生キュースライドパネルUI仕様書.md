...
### セクション構成
1. **ヘッダー領域**:
   - **タイトル**: 「再生キュー (Play Queue)」をネオンシアン色（`{StaticResource NeonCyanBrush}`）かつ太字で表示。
...
2. **キューリスト領域**:
   ...
   - **各トラックアイテムカード**:
     ...
     - **再生中インジケータ**: 再生中の楽曲には半透明黒オーバーレイ＋ネオンシアンの再生/一時停止アイコンを重畳表示。
     - **楽曲メタデータ**:
       - トラック名: 13pt、セミボールド、白色、長文時は末尾三点リーダー（Ellipsis）。クリックで即時再生。
       - アーティスト名: 11pt、セカンダリグレー、長文時Ellipsis。
     - **再生時間**: `mm:ss` 形式で右側に表示。
     - **インライン操作ボタン群**:
       - **上へ移動（▲）**: クリックで `MoveQueueItemUpCommand` を実行し、キュー内で1つ上に移動。
       - **下へ移動（▼）**: クリックで `MoveQueueItemDownCommand` を実行し、キュー内で1つ下に移動。
