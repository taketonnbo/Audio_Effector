...
flowchart TD
    ...
    subgraph Workspace["中央メインワークスペース (Main Workspace)"]
        ...
        ViewArtists["ArtistsView<br>(アーティスト一覧)"]
        ViewFolders["FolderView<br>(フォルダーツリー)"]
        ViewPlaylists["PlaylistSelectorView<br>(プレイリストカード一覧)"]
        ViewRecent["RecentView<br>(再生履歴タイムライン)"]
        ViewEQ["EqualizerView<br>(10バンドEQ画面)"]
        ViewDevice["DeviceSyncView<br>(外部機器同期画面)"]

        subgraph SingleViews["詳細画面 (Single Views)"]
            ViewPlaylistTracks["PlaylistTracksView<br>(プレイリスト詳細 / お気に入り)"]
            ViewAlbumDetail["AlbumDetailView<br>(アルバム詳細 / インライン展開)"]
            ViewArtistDetail["ArtistDetailView<br>(アーティスト所属アルバム・楽曲)"]
        end
    end
...
flowchart LR
    ...
    subgraph Overlays["ドロワー / オーバーレイ"]
        QueuePanel["PlayQueueSidePanel<br>(再生キュースライドパネル)"]
        InfoPanel["AlbumInfoPanel<br>(アルバム情報パネル)"]
    end

    subgraph Modals["モーダルダイアログ群"]
        DlgSettings["SettingsDialog<br>(設定ダイアログ)"]
        DlgDeviceMgr["DeviceManagerDialog<br>(機器ストレージ管理)"]
        DlgPlaylistSelect["PlaylistSelectionDialog<br>(プレイリスト追加先選択)"]
        DlgInputBox["InputBox<br>(名称入力・変更)"]
        DlgTagEditor["TagEditorDialog<br>(楽曲メタ情報・タグ編集)"]
    end
    ...
    %% オーバーレイ開閉
    MainUI -->|開閉トグル| QueuePanel
    MainUI -->|タブ切替| InfoPanel

    %% ダイアログ起動
    MainUI -->|ツールメニュー| DlgSettings
    MainUI -->|プロパティ編集| DlgTagEditor
    MainUI -->|プレイリスト追加| DlgPlaylistSelect
    MainUI -->|名前変更| DlgInputBox
    MainUI -->|機器管理ボタン| DlgDeviceMgr

    %% ミニプレイヤー切り替え
    MainUI -->|最小化または切替ボタン| MiniPlayer
    MiniPlayer -->|復帰操作| MainUI
```
