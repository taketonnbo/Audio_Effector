...
public class TrackTests
{
    ...
    /// <summary>
    /// コンストラクタ呼び出し時にタイトルやアーティストが空文字・空白の場合、ファイル名やUnknownへ適切にフォールバックされるかを検証します。
    /// </summary>
    [Fact]
    public void コンストラクタ_タイトルやアーティストが空の場合_ファイル名やUnknownにフォールバックすること()
    {
        // Arrange
        var audioPath = AudioPath.Create(@"C:\Music\mystery_track.mp3");
        ...
        var sut = new Track(
            ...
            artist: "",
            album: null!,
            duration: TimeSpan.FromSeconds(-30)); // 負の再生時間

        // Assert
        Assert.Equal("mystery_track.mp3", sut.Title);
        Assert.Equal("Unknown Artist", sut.Artist);
        Assert.Equal("Unknown Album", sut.Album);
